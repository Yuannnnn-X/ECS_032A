...
def read_gdp_data(happy_dict): # the function is used to read the tsv file
    ...
    for line in outfile: # print all the lines required
        line = line.strip() # strip the space
        ...
        if country not in happiness: # if the country is not in the file
            continue
        else: # if the country is in the file
            print(country+","+PopulationInMillions+","+GDPperCapita+","+happiness[country])
    return

def lookup_happiness_by_country(happy_dict): # print the countries satisfying the requirement
    while True: # make the while loop of the user's input
        UserInput = input("Enter a country to lookup or 'done' to exit:")
        if UserInput == "done": # when the user prompts done
            break
        if UserInput in happiness: # when the input is in the dictionary
            HappinessIndex = happiness[UserInput]
            print(HappinessIndex)
        else: # when the input is not in the dictionary
